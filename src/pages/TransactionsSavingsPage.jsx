...
const TransactionsSavingsPage = () => {
  ...
  useEffect(() => {
    ...
    fetch("/populateDatabase.json")
      ...
      .then(users => {
        ...
        const account = foundUser.accounts.find(acc => acc.accountType === 'savings');
  ...
  if (!savingsAccount) return <p>Compte Épargne introuvable</p>;

  const { title, amount, description = "" } = savingsAccount;
  ...
  return (
    <div>
      ...
      <section className="account-transaction-general">
        <TransactionHistory transactions={savingsAccount.transactions || []} />
      </section>
